...
export const emailTemplates: EmailTemplates = {
	welcome: (name: string, email: string, token: string) => ({
		subject: 'Welcome to Cavarates!',
		html: `
		${emailStyles}
		<div class="container">
			<img src="https://www.cavarates.com/logo.png" alt="cavarates Logo" class="logo" />
			<h1>Welcome to Cavarates, ${name}!</h1>
			<p>Thank you for registering with us. We're excited to have you on board!</p>
			<h4>Your Verification Token is: <strong>${token}</strong></h4>
			<p>Verify your email by clicking the button below:</p>
			<a href="https://cavarates.com/auth/verify?email=${email}" class="button">Verify Email</a>
			<p>If you have any questions, feel free to contact our support team.</p>
			<div class="footer">
				<p>Best regards,<br>The Cavarates Team</p>
			</div>
		</div>
		`,
	}),
...
export const sendEmail = async <T extends keyof EmailData>(
	...
	try {
		...
		const mailOptions = {
			from: `"Cavarates" <${
				process.env.EMAIL_FROM || 'noreply@cavarates.com'
			}>`,
			replyTo: `"Cavarates Support" <${
				process.env.EMAIL_FROM || 'noreply@cavarates.com'
			}>`,
			to,
			subject,
